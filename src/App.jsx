...
// Fonction utilitaire sécurisée (Anti-Crash)
...
export default function App() {
  ...
  useEffect(() => {
    const onboarded = localStorage.getItem('imperium_onboarded') === 'true';
...
function Dashboard({ onNavigate }) {
  // Récupération sécurisée des données (évite le crash si données corrompues)
  const [balance, setBalance] = useState(() => {
    try {
      return JSON.parse(localStorage.getItem('imperium_balance') || "0");
    } catch { return 0; }
  });

  const [transactions, setTransactions] = useState(() => {
    try {
      return JSON.parse(localStorage.getItem('imperium_transactions') || "[]");
    } catch { return []; }
  });
...
function SkillsScreen({ onBack }) {
    ...
    // Récupération sécurisée de la zone (Fallback si erreur)
        ...
        <div className="min-h-[100dvh] w-full max-w-md mx-auto bg-dark text-gray-200 font-sans flex flex-col animate-in slide-in-from-right duration-300">
            <div className="px-5 py-4 bg-[#151515] border-b border-white/5 pt-[env(safe-area-inset-top)] sticky top-0 z-10">
                ...
                <div className="flex items-center gap-2 mt-2">
                    ...
                    <span className="text-[10px] text-gray-400 uppercase">Marché : {userZone ? userZone.name : "Monde"}</span>
                </div>
            </div>
        ...
        <div className="min-h-[100dvh] w-full max-w-md mx-auto bg-dark text-gray-200 font-sans flex flex-col animate-in slide-in-from-right duration-300">
            ...
            <div className="p-5 space-y-8">
                <div className="bg-[#111] border border-white/5 rounded-xl p-5">
                    <div className="flex items-center gap-3 mb-3"><div className="p-2 bg-blue-900/20 text-blue-400 rounded-lg"><Download className="w-5 h-5"/></div><div><h3 className="text-sm font-bold text-gray-200">Sauvegarder l'Empire</h3><p className="text-[10px] text-gray-500">Générez un code unique.</p></div></div>
                ...
                <div className="bg-[#111] border border-white/5 rounded-xl p-5">
                    <div className="flex items-center gap-3 mb-3"><div className="p-2 bg-green-900/20 text-green-400 rounded-lg"><Upload className="w-5 h-5"/></div><div><h3 className="text-sm font-bold text-gray-200">Restaurer les données</h3><p className="text-[10px] text-gray-500">Collez le code ici.</p></div></div>
