    ...
    return (
        <div className="fixed inset-0 bg-black z-[200] flex flex-col items-center justify-center p-6 text-center">
            <div className="w-full max-w-sm">
                <div className="w-24 h-24 bg-red-900/10 rounded-full flex items-center justify-center mx-auto mb-6 border border-red-500/30 animate-pulse"><Lock className="w-10 h-10 text-red-500" /></div>
                <h1 className="text-2xl font-serif text-white font-bold mb-2 uppercase tracking-widest">Zone Restreinte</h1>
                <p className="text-gray-500 text-xs mb-8 leading-relaxed">Cette application est en phase de test classifiée.<br/>L'accès est limité au personnel autorisé disposant d'une Clé Impériale.</p>
                <form onSubmit={checkCode} className="space-y-4">
                    <div className="relative"><Key className="absolute left-3 top-3.5 w-4 h-4 text-gray-500" /><input type="text" value={code} onChange={(e) => setCode(e.target.value)} className={`w-full bg-[#111] border rounded-lg pl-10 pr-4 py-3 text-white font-mono text-center uppercase tracking-widest focus:outline-none transition-all ${error ? 'border-red-500 animate-shake' : 'border-white/20 focus:border-gold'}`} placeholder="XXX-XXXXX-00" autoFocus /></div>
                    <button type="submit" disabled={!code || loading} className={`w-full font-bold py-4 rounded-lg uppercase tracking-widest text-xs transition-all flex items-center justify-center gap-2 ${loading ? 'bg-gray-800 text-gray-500' : 'bg-white text-black hover:bg-gray-200'}`}>{loading ? <Loader2 className="w-4 h-4 animate-spin"/> : <Unlock className="w-4 h-4"/>}{loading ? "Vérification..." : "Déverrouiller"}</button>
                </form>
                {error && (<div className="mt-6 p-3 bg-red-900/20 border border-red-500/20 rounded text-red-400 text-xs flex items-center justify-center gap-2 animate-in fade-in slide-in-from-top-2"><AlertTriangle className="w-4 h-4" /> Code invalide ou expiré.</div>)}
...
function Dashboard({ onNavigate }) {
  ...
  const spentToday = transactions.filter(t => t.type === 'expense' && t.date === todayStr).reduce((acc, t) => acc + t.amount, 0);
  ...
  if (spentToday > dailyAllocation) dailyStatus = { text: "Critique ⚠️", color: "text-red-500" };
  else if (spentToday > dailyAllocation * 0.8) dailyStatus = { text: "Attention", color: "text-orange-500" };
  else if (spentToday > 0) dailyStatus = { text: "Excellent", color: "text-green-500" };
  ...
  const handleUndoTransaction = (txId) => {
      ...
      if(!tx) return;
      if(tx.type === 'expense') setBalance(balance + tx.amount);
    ...
    <div className="min-h-[100dvh] w-full max-w-md mx-auto bg-dark text-gray-200 font-sans pb-40 flex flex-col relative shadow-2xl">
      ...
      <main className="w-full px-4 grid gap-3 mt-4">
        ...
        <div className={`bg-[#111] border rounded-xl p-0 relative overflow-hidden transition-colors ${availableCash < 0 ? 'border-red-500/50 bg-red-900/10' : 'border-white/5'}`}>
            ...
            <div className="w-full h-1 bg-gray-900 mt-2"><div className="h-full bg-blue-500/30" style={{ width: `${monthProgress}%` }}></div></div>
            <div className="grid grid-cols-2 divide-x divide-white/10 bg-white/5">
                <div className="p-3 text-center"><p className="text-[9px] text-gray-500 uppercase tracking-widest mb-1">Ration (J-{daysRemaining})</p><p className={`text-lg font-bold ${dailyAllocation < 500 ? 'text-orange-500' : 'text-white'}`}>{formatMoney(dailyAllocation)} <span className="text-[10px] font-normal text-gray-500">{currency}</span></p></div>
                <div className="p-3 text-center relative"><p className="text-[9px] text-gray-500 uppercase tracking-widest mb-1">Dépensé 24h</p><p className={`text-lg font-bold ${spentToday > dailyAllocation ? 'text-red-500' : 'text-white'}`}>{formatMoney(spentToday)} <span className="text-[10px] font-normal text-gray-500">{currency}</span></p><span className={`absolute top-2 right-2 w-2 h-2 rounded-full ${dailyStatus.color === 'text-green-500' ? 'bg-green-500' : dailyStatus.color === 'text-red-500' ? 'bg-red-500' : 'bg-gray-700'}`}></span></div>
            </div>
             <div className="p-2 text-center bg-black/40 border-t border-white/5"><p className={`text-[10px] ${dailyStatus.color} font-bold uppercase tracking-widest`}>STATUT : {dailyStatus.text}</p></div>
        </div>
        ...
        <div onClick={() => setIsBunkerModalOpen(true)} className="bg-blue-900/10 border border-blue-500/30 rounded-xl p-4 flex items-center justify-between cursor-pointer active:scale-[0.98] transition-all hover:bg-blue-900/20 group relative overflow-hidden">
             ...
             <div className="flex items-center gap-4 relative z-10">
                 ...
                 <div><p className="text-[9px] text-blue-400 uppercase tracking-widest font-bold mb-1">Coffre-Fort Wave</p><h3 className="text-xl font-bold text-white font-serif tracking-wide">{formatMoney(totalBunker)} {currency}</h3><p className="text-[9px] text-gray-400">Ne pas toucher sauf urgence.</p></div>
             </div>
             <ChevronRight className="w-5 h-5 text-blue-500/50 group-hover:text-blue-400 relative z-10" />
        </div>

        {debtToPay && (<div className="bg-[#1a0f0f] p-3 rounded-xl border border-red-900/30 animate-in slide-in-from-right flex items-center gap-3"><div className="p-2 bg-red-900/20 text-red-500 rounded-lg shrink-0"><AlertTriangle className="w-4 h-4"/></div><div className="flex-1"><p className="text-[9px] text-red-400 uppercase tracking-widest font-bold">Dette Prioritaire</p><p className="text-xs text-white font-bold">{debtToPay.name} : {formatMoney(debtToPay.amount)} {currency}</p></div><button onClick={() => onNavigate('debts')} className="text-xs text-red-400 font-bold uppercase border border-red-900/50 px-3 py-1.5 rounded bg-red-900/10">Payer</button></div>)}

        {/* MENU RAPIDE */}
        <div className="grid grid-cols-4 gap-2 mt-2">
            <button onClick={() => onNavigate('protocols')} className="bg-[#111] border border-white/5 hover:border-gold/30 rounded-xl p-2 flex flex-col items-center justify-center gap-1 active:scale-95 transition-all"><div className="text-indigo-500"><Repeat className="w-4 h-4"/></div><span className="text-[9px] font-bold text-gray-400 uppercase">Protocoles</span></button>
            <button onClick={() => onNavigate('debts')} className="bg-[#111] border border-white/5 hover:border-gold/30 rounded-xl p-2 flex flex-col items-center justify-center gap-1 active:scale-95 transition-all"><div className="text-purple-500"><Scroll className="w-4 h-4"/></div><span className="text-[9px] font-bold text-gray-400 uppercase">Registre</span></button>
            <button onClick={() => onNavigate('goals')} className="bg-[#111] border border-white/5 hover:border-gold/30 rounded-xl p-2 flex flex-col items-center justify-center gap-1 active:scale-95 transition-all"><div className="text-blue-500"><Target className="w-4 h-4"/></div><span className="text-[9px] font-bold text-gray-400 uppercase">Cibles</span></button>
             <button onClick={() => onNavigate('trophies')} className="bg-[#111] border border-white/5 hover:border-gold/30 rounded-xl p-2 flex flex-col items-center justify-center gap-1 active:scale-95 transition-all"><div className="text-gold"><Trophy className="w-4 h-4"/></div><span className="text-[9px] font-bold text-gray-400 uppercase">Succès</span></button>
        </div>
        ...
        <div className="mt-4 bg-[#111] border border-white/5 rounded-xl p-4 flex gap-3 items-start opacity-80">
            ...
            <div><p className="text-xs text-gray-300 italic font-serif leading-relaxed">"{dailyQuote.text}"</p><p className="text-[10px] text-gold mt-2 font-bold uppercase">— {dailyQuote.author}</p></div>
        </div>
      </main>
      ...
      {isBunkerModalOpen && (
        <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/90 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-[#050b1a] border-t border-blue-500/30 w-full max-w-md rounded-t-2xl p-6 shadow-2xl animate-in slide-in-from-bottom-full duration-300 pb-10 mb-[env(safe-area-inset-bottom)] relative overflow-hidden">
             ...
             <div className="flex justify-between items-center mb-6"><div className="flex items-center gap-2"><Smartphone className="w-5 h-5 text-blue-400"/><h2 className="font-serif text-blue-400 text-sm tracking-widest uppercase font-bold">Compte Wave (Bunker)</h2></div><button onClick={() => setIsBunkerModalOpen(false)}><X className="w-5 h-5 text-gray-500" /></button></div>
             <div className="text-center mb-6"><p className="text-[10px] text-gray-400 uppercase tracking-widest mb-1">Total Sécurisé</p><h2 className="text-4xl font-bold text-white font-serif">{formatMoney(totalBunker)} {currency}</h2><p className="text-xs text-blue-300 mt-2 px-6">Pour ajouter de l'argent ici, vous devez faire un dépôt physique sur votre compte Wave.</p></div>
             <div className="space-y-4">
                 ...
                 <div className="flex gap-3"><button onClick={() => handleBunkerAction('withdraw')} className="flex-1 bg-red-900/10 hover:bg-red-900/20 text-red-500 border border-red-900/30 py-4 rounded-lg font-bold text-xs uppercase flex flex-col items-center justify-center gap-1 transition-colors"><Unlock className="w-4 h-4"/><span>Urgence (Retrait)</span></button><button onClick={() => handleBunkerAction('deposit')} className="flex-1 bg-blue-600 text-white py-4 rounded-lg font-bold text-xs uppercase flex flex-col items-center justify-center gap-1 hover:bg-blue-500 transition-colors shadow-[0_0_20px_rgba(59,130,246,0.2)]"><Lock className="w-4 h-4"/><span>Confirmer le Dépôt</span></button></div>
             </div>
          </div>
        </div>
      )}

      {/* MODAL HISTORIQUE RESTAURÉ */}
      {showHistory && (
          <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/90 backdrop-blur-sm animate-in fade-in">
              <div className="bg-[#111] border-t border-white/10 w-full max-w-md rounded-t-2xl p-6 shadow-2xl h-[80vh] flex flex-col animate-in slide-in-from-bottom duration-300">
                  <div className="flex justify-between items-center mb-6"><div className="flex items-center gap-2"><History className="w-5 h-5 text-gold"/><h2 className="font-serif text-white text-sm tracking-widest uppercase font-bold">Journal de Bord</h2></div><button onClick={() => setShowHistory(false)}><X className="w-5 h-5 text-gray-500" /></button></div>
                  <div className="flex-1 overflow-y-auto custom-scrollbar space-y-3 pb-10">
                    {transactions.length === 0 ? <p className="text-gray-500 text-xs text-center mt-10">Le journal est vierge.</p> : transactions.map(tx => (
                        <div key={tx.id} className="flex justify-between items-center p-3 bg-[#1a1a1a] rounded-lg border border-white/5">
                            <div><p className="text-xs text-white font-bold">{tx.desc}</p><p className="text-[10px] text-gray-500">{tx.date} • {tx.type === 'expense' ? (tx.category === 'want' ? 'Futilité' : 'Besoin') : 'Revenu'}</p></div>
                            <div className="flex items-center gap-3"><span className={`text-sm font-bold ${tx.type === 'expense' ? 'text-red-500' : 'text-green-500'}`}>{tx.type === 'expense' ? '-' : '+'}{formatMoney(tx.amount)}</span><button onClick={() => handleUndoTransaction(tx.id)} className="p-2 bg-red-900/10 text-red-500 rounded hover:bg-red-900/30 transition-colors"><Trash2 className="w-4 h-4" /></button></div>
                        </div>
                    ))}
                  </div>
              </div>
          ...
          <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/95 backdrop-blur-md p-6 animate-in fade-in">
              <div className="bg-[#111] border border-gold/40 w-full max-w-sm rounded-2xl p-6 shadow-[0_0_50px_rgba(212,175,55,0.1)] relative overflow-hidden">
                  ...
                  <div className="flex flex-col items-center text-center">
                      <div className="w-16 h-16 bg-gold/10 rounded-full flex items-center justify-center mb-4 border border-gold/40"><Coins className="w-8 h-8 text-gold" /></div>
                      <h3 className="text-gold font-serif text-xl font-bold mb-2 tracking-wide uppercase">L'Impôt Impérial</h3>
                      <p className="text-gray-400 text-sm mb-6 leading-relaxed">La règle d'or est de se payer en premier. L'Empire réclame <span className="text-white font-bold">20%</span> de ce revenu pour le <span className="text-blue-400 font-bold">Bunker (Wave)</span>.</p>
                      <div className="w-full bg-gray-900 rounded-lg p-4 mb-6 border border-white/5"><div className="flex justify-between text-xs mb-2"><span className="text-gray-500">Revenu Total</span><span className="text-white font-bold">{formatMoney(pendingTransaction.value)} {currency}</span></div><div className="flex justify-between text-sm"><span className="text-gold font-bold">Prélèvement (20%)</span><span className="text-gold font-bold">-{formatMoney(Math.floor(pendingTransaction.value * 0.2))} {currency}</span></div></div>
                      <div className="flex gap-3 w-full"><button onClick={() => processIncomeWithTax(false)} className="flex-1 py-3 rounded-lg border border-white/10 text-gray-500 text-xs font-bold uppercase hover:bg-white/5">Faiblesse (0%)</button><button onClick={() => processIncomeWithTax(true)} className="flex-1 py-3 rounded-lg bg-gold text-black text-xs font-bold uppercase hover:bg-yellow-400">Investir (20%)</button></div>
                  </div>
              </div>
          </div>
      )}
...
function ProtocolsScreen({ onBack }) {
    ...
    const addProtocol = (e) => { e.preventDefault(); if (!newName || !newAmount) return; setProtocols([...protocols, { id: Date.now(), name: newName, amount: parseFloat(newAmount), type, freq }]); setNewName(""); setNewAmount(""); setFreq('monthly'); };
    const deleteProtocol = (id) => { setProtocols(protocols.filter(p => p.id !== id)); };
    const calculateMonthly = (p) => { const frequency = FREQUENCIES.find(f => f.id === (p.freq || 'monthly')); return p.amount * (frequency ? frequency.factor : 1); };
    ...
    return (
        <PageTransition><div className="h-[100dvh] w-full max-w-md mx-auto bg-dark text-gray-200 font-sans flex flex-col overflow-hidden">
            <div className="shrink-0 px-5 py-4 bg-[#151515] border-b border-white/5 pt-[env(safe-area-inset-top)] z-10"><button onClick={onBack} className="flex items-center gap-2 text-gray-500 hover:text-white mb-4 mt-2"><ArrowLeft className="w-4 h-4" /> <span className="text-xs uppercase tracking-widest">Retour au QG</span></button><h1 className="text-2xl font-serif text-white font-bold">Protocoles</h1></div>
            <div className="flex-1 overflow-y-auto p-5 pb-20 custom-scrollbar"><div className="mb-6 bg-[#111] rounded-xl border border-white/10 p-5"><div className="flex items-center gap-2 mb-4 opacity-70"><Infinity className="w-4 h-4 text-gold" /><h3 className="text-xs font-bold uppercase tracking-widest">Projection Mensuelle</h3></div><div className="flex justify-between items-center mb-2"><span className="text-xs text-gray-500">Revenus Projetés</span><span className="text-xs font-bold text-green-500">+{formatMoney(fixedIncome)} {currency}</span></div><div className="flex justify-between items-center mb-4"><span className="text-xs text-gray-500">Charges Projetées</span><span className="text-xs font-bold text-red-500">-{formatMoney(fixedExpenses)} {currency}</span></div><div className="pt-4 border-t border-white/10 flex justify-between items-center"><span className="text-xs font-bold text-white uppercase">Cash-Flow Net</span><span className={`text-xl font-bold font-serif ${cashFlow >= 0 ? 'text-gold' : 'text-red-500'}`}>{cashFlow > 0 ? '+' : ''}{formatMoney(cashFlow)} <span className="text-xs">{currency}</span></span></div></div><div className="space-y-3">{protocols.length === 0 && <p className="text-center text-gray-600 text-xs mt-10">Aucun protocole actif.</p>}{protocols.map(p => { const fLabel = FREQUENCIES.find(f => f.id === (p.freq || 'monthly'))?.label.split('(')[0] || 'Mensuel'; return (<div key={p.id} className="bg-[#111] border border-white/5 p-4 rounded-xl flex justify-between items-center group"><div className="flex items-center gap-3"><div className={`p-2 rounded-lg ${p.type === 'expense' ? 'bg-red-900/10 text-red-500' : 'bg-green-900/10 text-green-500'}`}>{p.type === 'expense' ? <CalendarClock className="w-5 h-5"/> : <Briefcase className="w-5 h-5"/>}</div><div><h3 className="text-sm font-bold text-gray-200">{p.name}</h3><p className="text-[10px] text-gray-500">{fLabel}</p></div></div><div className="flex items-center gap-4"><span className={`text-sm font-bold ${p.type === 'expense' ? 'text-red-500' : 'text-green-500'}`}>{formatMoney(p.amount)}</span><button onClick={() => deleteProtocol(p.id)} className="text-gray-700 hover:text-red-500"><X className="w-4 h-4"/></button></div></div>); })}</div></div>
            <div className="shrink-0 p-4 bg-[#0a0a0a] border-t border-white/10 pb-[calc(2rem+env(safe-area-inset-bottom))] max-w-md mx-auto w-full"><div className="flex bg-black p-1 rounded-lg mb-3 border border-white/5"><button onClick={() => setType('expense')} className={`flex-1 py-2 text-[10px] font-bold uppercase rounded transition-colors ${type === 'expense' ? 'bg-red-900/50 text-red-200' : 'text-gray-600'}`}>Charge</button><button onClick={() => setType('income')} className={`flex-1 py-2 text-[10px] font-bold uppercase rounded transition-colors ${type === 'income' ? 'bg-green-900/50 text-green-200' : 'text-gray-600'}`}>Rente</button></div><form onSubmit={addProtocol} className="flex flex-col gap-3"><input type="text" value={newName} onChange={(e) => setNewName(e.target.value)} placeholder="Nom (ex: Loyer, Salaire...)" className="w-full bg-[#111] border border-white/10 rounded-lg px-4 py-3 text-white text-sm focus:border-gold focus:outline-none" /><div className="flex gap-2"><input type="number" value={newAmount} onChange={(e) => setNewAmount(e.target.value)} placeholder="Montant" className="flex-1 bg-[#111] border border-white/10 rounded-lg px-4 py-3 text-white text-sm focus:border-gold focus:outline-none" /><select value={freq} onChange={(e) => setFreq(e.target.value)} className="bg-[#111] border border-white/10 rounded-lg px-2 text-white text-xs focus:border-gold outline-none w-24">{FREQUENCIES.map(f => <option key={f.id} value={f.id}>{f.label.split('(')[1].replace(')', '')}</option>)}</select><button type="submit" disabled={!newName || !newAmount} className="bg-white/10 text-white font-bold px-4 rounded-lg disabled:opacity-50 hover:bg-white/20 transition-colors"><Plus className="w-5 h-5" /></button></div></form></div>
        </div></PageTransition>
    );
}
...
function DebtsScreen({ onBack }) {
    ...
    const addEntry = (e) => { e.preventDefault(); if (!newName || !newAmount) return; setDebts([...debts, { id: Date.now(), name: newName, amount: parseFloat(newAmount), type }]); setNewName(""); setNewAmount(""); };
    const settleEntry = (item) => { if(item.type === 'owe') { if(balance < item.amount) return alert("Trésorerie insuffisante pour honorer cette dette."); setBalance(balance - item.amount); } else { setBalance(balance + item.amount); } setDebts(debts.filter(d => d.id !== item.id)); };
    const deleteEntry = (id) => { if(confirm("Supprimer cette entrée ? Cela n'affectera pas votre solde.")) { setDebts(debts.filter(d => d.id !== id)); } };
    ...
    return (
        <PageTransition><div className="h-[100dvh] w-full max-w-md mx-auto bg-dark text-gray-200 font-sans flex flex-col overflow-hidden">
            <div className="shrink-0 px-5 py-4 bg-[#151515] border-b border-white/5 pt-[env(safe-area-inset-top)] z-10"><button onClick={onBack} className="flex items-center gap-2 text-gray-500 hover:text-white mb-4 mt-2"><ArrowLeft className="w-4 h-4" /> <span className="text-xs uppercase tracking-widest">Retour au QG</span></button><h1 className="text-2xl font-serif text-white font-bold">Le Registre</h1></div>
            <div className="flex-1 overflow-y-auto p-5 pb-20 custom-scrollbar"><div className="grid grid-cols-2 gap-3 mb-6"><div className="bg-red-900/10 border border-red-500/20 p-4 rounded-xl"><p className="text-[10px] text-red-400 uppercase tracking-widest mb-1">Dettes (Tributs)</p><p className="text-xl font-bold text-white">{formatMoney(totalOwe)} {currency}</p></div><div className="bg-green-900/10 border border-green-500/20 p-4 rounded-xl"><p className="text-[10px] text-green-400 uppercase tracking-widest mb-1">Créances (Butin)</p><p className="text-xl font-bold text-white">{formatMoney(totalOwed)} {currency}</p></div></div><div className="space-y-3">{debts.length === 0 && <p className="text-center text-gray-600 text-xs mt-10">Le registre est vierge.</p>}{debts.map(item => (<div key={item.id} className={`p-4 rounded-xl border flex justify-between items-center ${item.type === 'owe' ? 'bg-[#111] border-red-500/20' : 'bg-[#111] border-green-500/20'}`}><div className="flex items-center gap-3"><div className={`p-2 rounded-lg ${item.type === 'owe' ? 'bg-red-900/20 text-red-500' : 'bg-green-900/20 text-green-500'}`}>{item.type === 'owe' ? <UserMinus className="w-5 h-5"/> : <UserPlus className="w-5 h-5"/>}</div><div><h3 className="text-sm font-bold text-gray-200">{item.name}</h3><p className="text-[10px] text-gray-500">{formatMoney(item.amount)} {currency}</p></div></div><div className="flex items-center gap-2"><button onClick={() => settleEntry(item)} className={`px-3 py-1 rounded text-[10px] font-bold uppercase border ${item.type === 'owe' ? 'border-red-500 text-red-500 hover:bg-red-900/20' : 'border-green-500 text-green-500 hover:bg-green-900/20'}`}>{item.type === 'owe' ? "Honorer" : "Encaisser"}</button><button onClick={() => deleteEntry(item.id)} className="p-2 text-gray-600 hover:text-red-500 transition-colors"><Trash2 className="w-4 h-4"/></button></div></div>))}</div></div>
            <div className="shrink-0 p-4 bg-[#0a0a0a] border-t border-white/10 pb-[calc(2rem+env(safe-area-inset-bottom))] max-w-md mx-auto w-full"><div className="flex bg-black p-1 rounded-lg mb-3 border border-white/5"><button onClick={() => setType('owe')} className={`flex-1 py-2 text-[10px] font-bold uppercase rounded transition-colors ${type === 'owe' ? 'bg-red-900/50 text-red-200' : 'text-gray-600'}`}>Je Dois (Dette)</button><button onClick={() => setType('owed')} className={`flex-1 py-2 text-[10px] font-bold uppercase rounded transition-colors ${type === 'owed' ? 'bg-green-900/50 text-green-200' : 'text-gray-600'}`}>On me Doit (Créance)</button></div><form onSubmit={addEntry} className="flex flex-col gap-3"><input type="text" value={newName} onChange={(e) => setNewName(e.target.value)} placeholder="Nom (ex: Moussa)" className="w-full bg-[#111] border border-white/10 rounded-lg px-4 py-3 text-white text-sm focus:border-gold focus:outline-none" /><div className="flex gap-2"><input type="number" value={newAmount} onChange={(e) => setNewAmount(e.target.value)} placeholder="Montant" className="flex-1 bg-[#111] border border-white/10 rounded-lg px-4 py-3 text-white text-sm focus:border-gold focus:outline-none" /><button type="submit" disabled={!newName || !newAmount} className="bg-white/10 text-white font-bold px-6 py-3 rounded-lg disabled:opacity-50 hover:bg-white/20 transition-colors"><Plus className="w-5 h-5" /></button></div></form></div>
        </div></PageTransition>
    );
}
...
function GoalsScreen({ onBack }) {
    ...
    const addGoal = (e) => { e.preventDefault(); if (!newGoalName || !newGoalTarget) return; setGoals([...goals, { id: Date.now(), title: newGoalName, target: parseFloat(newGoalTarget), deadline: newGoalDeadline, current: 0 }]); setNewGoalName(""); setNewGoalTarget(""); setNewGoalDeadline(""); };
    const deleteGoal = (id) => { setGoals(goals.filter(g => g.id !== id)); };
    const handleAllocation = (type) => { if(!allocAmount || !selectedGoal) return; const val = parseFloat(allocAmount); const updatedGoals = goals.map(g => { if (g.id === selectedGoal.id) { return { ...g, current: type === 'deposit' ? g.current + val : Math.max(0, g.current - val) }; } return g; }); setGoals(updatedGoals); setAllocAmount(""); setSelectedGoal(null); };
    return (
        <PageTransition><div className="h-[100dvh] w-full max-w-md mx-auto bg-dark text-gray-200 font-sans flex flex-col overflow-hidden">
            <div className="shrink-0 px-5 py-4 bg-[#151515] border-b border-white/5 pt-[env(safe-area-inset-top)] z-10"><button onClick={onBack} className="flex items-center gap-2 text-gray-500 hover:text-white mb-4 mt-2"><ArrowLeft className="w-4 h-4" /> <span className="text-xs uppercase tracking-widest">Retour au QG</span></button><h1 className="text-2xl font-serif text-white font-bold">Cibles</h1></div>
            <div className="flex-1 overflow-y-auto p-5 pb-20 custom-scrollbar"><div className="space-y-4">{goals.map(goal => { const percent = Math.min(100, Math.round((goal.current / goal.target) * 100)); const daysLeft = getDaysLeft(goal.deadline); return (<div key={goal.id} className="bg-[#111] border border-white/5 p-4 rounded-xl"><div className="flex justify-between items-start mb-2"><div className="flex items-center gap-3"><div className={`p-2 rounded-lg ${percent >= 100 ? 'bg-green-900/20 text-green-500' : 'bg-blue-900/20 text-blue-500'}`}><Target className="w-5 h-5"/></div><div><h3 className="text-sm font-bold text-gray-200">{goal.title}</h3><p className="text-[10px] text-gray-500">{percent}% Sécurisé</p></div></div><button onClick={() => deleteGoal(goal.id)} className="text-gray-700 hover:text-red-500"><X className="w-4 h-4"/></button></div>{daysLeft !== null && (<div className={`text-[10px] font-bold mb-2 flex items-center gap-1 ${daysLeft < 3 ? 'text-red-500 animate-pulse' : 'text-gray-500'}`}><Clock className="w-3 h-3" /> {daysLeft > 0 ? `${daysLeft} Jours restants` : "Date dépassée"}</div>)}<div className="w-full bg-gray-900 rounded-full h-2 mb-2"><div className={`h-2 rounded-full transition-all duration-500 ${percent >= 100 ? 'bg-green-500' : 'bg-blue-500'}`} style={{ width: `${percent}%` }}></div></div><div className="flex justify-between items-center mb-4"><span className="text-xs font-bold text-white">{formatMoney(goal.current)} {currency}</span><span className="text-[10px] text-gray-500">Obj: {formatMoney(goal.target)}</span></div><div className="flex gap-2"><button onClick={() => setSelectedGoal(goal)} className="flex-1 bg-white/5 hover:bg-white/10 border border-white/10 py-2 rounded text-xs font-bold uppercase">Gérer</button></div></div>); })}</div></div>
            {selectedGoal && (<div className="fixed inset-0 z-50 flex items-center justify-center bg-black/90 backdrop-blur-sm p-6 animate-in fade-in"><div className="bg-[#1a1a1a] border border-white/10 w-full max-w-sm rounded-2xl p-6 shadow-2xl relative"><button onClick={() => setSelectedGoal(null)} className="absolute top-4 right-4 text-gray-500"><X className="w-5 h-5"/></button><h3 className="text-white font-serif text-lg mb-4 text-center">Gérer: {selectedGoal.title}</h3><input type="number" value={allocAmount} onChange={(e) => setAllocAmount(e.target.value)} className="w-full bg-black border border-white/20 rounded-lg p-3 text-white text-center text-xl focus:border-gold focus:outline-none mb-4" placeholder="Montant" autoFocus /><div className="flex gap-3"><button onClick={() => handleAllocation('withdraw')} className="flex-1 bg-red-900/20 text-red-500 border border-red-500/30 py-3 rounded-lg font-bold text-xs uppercase flex items-center justify-center gap-2"><Unlock className="w-4 h-4"/> Retirer</button><button onClick={() => handleAllocation('deposit')} className="flex-1 bg-green-900/20 text-green-500 border border-green-500/30 py-3 rounded-lg font-bold text-xs uppercase flex items-center justify-center gap-2"><Lock className="w-4 h-4"/> Verser</button></div></div></div>)}
            <div className="shrink-0 p-4 bg-[#0a0a0a] border-t border-white/10 pb-[calc(2rem+env(safe-area-inset-bottom))] max-w-md mx-auto w-full"><form onSubmit={addGoal} className="flex flex-col gap-3"><input type="text" value={newGoalName} onChange={(e) => setNewGoalName(e.target.value)} placeholder="Nom (ex: PC Gamer)" className="w-full bg-[#111] border border-white/10 rounded-lg px-4 py-3 text-white text-sm focus:border-gold focus:outline-none" /><div className="flex gap-2"><input type="number" value={newGoalTarget} onChange={(e) => setNewGoalTarget(e.target.value)} placeholder="Cible" className="flex-1 bg-[#111] border border-white/10 rounded-lg px-4 py-3 text-white text-sm focus:border-gold focus:outline-none" /><input type="date" value={newGoalDeadline} onChange={(e) => setNewGoalDeadline(e.target.value)} className="bg-[#111] border border-white/10 rounded-lg px-2 text-white text-xs focus:border-gold outline-none" /><button type="submit" disabled={!newGoalName || !newGoalTarget} className="bg-blue-600/80 text-white font-bold px-4 py-3 rounded-lg disabled:opacity-50 hover:bg-blue-500 transition-colors"><Plus className="w-5 h-5" /></button></div></form></div>
        </div></PageTransition>
    );
}
...
function StatsScreen({ onBack }) {
    ...
    const generateTrendData = () => {
        ...
        for (let i = 0; i < 30; i++) {
            ...
            dayTx.forEach(t => { if (t.type === 'income') current -= t.amount; else current += t.amount; });
        }
        return data;
    };
    ...
    return (
        <PageTransition><div className="h-[100dvh] w-full max-w-md mx-auto bg-dark text-gray-200 font-sans flex flex-col overflow-hidden">
            <div className="shrink-0 px-5 py-4 bg-[#151515] border-b border-white/5 pt-[env(safe-area-inset-top)] z-10"><button onClick={onBack} className="flex items-center gap-2 text-gray-500 hover:text-white mb-4 mt-2"><ArrowLeft className="w-4 h-4" /> <span className="text-xs uppercase tracking-widest">Retour au QG</span></button><h1 className="text-2xl font-serif text-white font-bold">Salle des Cartes</h1></div>
            <div className="flex-1 overflow-y-auto p-5 pb-20 custom-scrollbar"><div className="mb-6"><div className="flex justify-between items-center mb-2"><div className="flex items-center gap-2"><Activity className="w-4 h-4 text-gold"/><h3 className="text-xs font-bold uppercase tracking-widest text-white">Courbe de Puissance</h3></div><span className={`text-xs font-bold flex items-center gap-1 ${isTrendingUp ? 'text-green-500' : 'text-red-500'}`}>{isTrendingUp ? <TrendingUp className="w-3 h-3"/> : <TrendingDown className="w-3 h-3"/>}{isTrendingUp ? "Croissance" : "Déclin"}</span></div><PowerChart data={trendData} color={isTrendingUp ? "#22c55e" : "#ef4444"} /></div><div className="grid grid-cols-2 gap-3 mb-6"><div className="bg-[#111] p-4 rounded-xl border border-white/5"><p className="text-[10px] text-gray-500 uppercase tracking-widest mb-1">Total Dépensé</p><p className="text-xl font-bold text-white">{formatMoney(totalExpenses)} {currency}</p></div><div className="bg-[#111] p-4 rounded-xl border border-white/5"><p className="text-[10px] text-gray-500 uppercase tracking-widest mb-1">Ratio Futilité</p><p className={`text-xl font-bold ${wantPercent > 30 ? 'text-red-500' : 'text-green-500'}`}>{wantPercent}%</p></div></div><div className="bg-[#111] border border-white/5 rounded-xl p-6 mb-6"><h3 className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-6">Répartition Stratégique</h3><div className="mb-4"><div className="flex justify-between text-xs mb-2"><span className="text-white font-bold">Nécessités</span><span className="text-gray-400">{formatMoney(needs)} {currency}</span></div><div className="w-full bg-gray-900 rounded-full h-2"><div className="bg-white h-2 rounded-full" style={{ width: `${needPercent}%` }}></div></div></div><div><div className="flex justify-between text-xs mb-2"><span className="text-red-400 font-bold">Futilités (Plaisirs)</span><span className="text-gray-400">{formatMoney(wants)} {currency}</span></div><div className="w-full bg-gray-900 rounded-full h-2"><div className="bg-red-500 h-2 rounded-full shadow-[0_0_10px_rgba(239,68,68,0.5)]" style={{ width: `${wantPercent}%` }}></div></div></div></div><div className="bg-[#1a1a1a] border-l-2 border-gold p-4 rounded-r-lg"><div className="flex items-center gap-2 mb-2"><Shield className="w-4 h-4 text-gold" /><span className="text-xs font-bold text-gold uppercase tracking-widest">Rapport du Sergent</span></div><p className="text-sm text-gray-300 italic leading-relaxed">{totalExpenses === 0 ? "Aucune donnée. L'Empire est immobile." : wantPercent > 50 ? "DISCIPLINE REQUISE ! Vous gaspillez plus de la moitié de vos ressources. L'Empire va s'effondrer." : wantPercent > 20 ? "Attention. Les plaisirs grignotent le trésor." : "Excellent. Vos ressources sont allouées à la survie et à la conquête."}</p></div></div>
        </div></PageTransition>
    );
}
...
function TrophiesScreen({ onBack }) {
    ...
    const calculateStreak = () => { if (transactions.length === 0) return 0; const lastSin = transactions.find(t => t.type === 'expense' && t.category === 'want'); if (!lastSin) return Math.min(transactions.length, 30); const lastSinDate = new Date(lastSin.rawDate || Date.now()); const diffTime = Math.abs(new Date() - lastSinDate); return Math.ceil(diffTime / (1000 * 60 * 60 * 24)); };
    const streak = calculateStreak();
    return (
        <PageTransition><div className="h-[100dvh] w-full max-w-md mx-auto bg-dark text-gray-200 font-sans flex flex-col overflow-hidden">
            <div className="shrink-0 px-5 py-4 bg-[#151515] border-b border-white/5 pt-[env(safe-area-inset-top)] z-10"><button onClick={onBack} className="flex items-center gap-2 text-gray-500 hover:text-white mb-4 mt-2"><ArrowLeft className="w-4 h-4" /> <span className="text-xs uppercase tracking-widest">Retour au QG</span></button><h1 className="text-2xl font-serif text-white font-bold">Salle des Trophées</h1></div>
            <div className="flex-1 overflow-y-auto p-5 pb-20 custom-scrollbar"><div className="grid grid-cols-2 gap-4">{TROPHIES_DATA.map(trophy => { const isUnlocked = trophy.condition(balance, streak, projects); const TrophyIcon = trophy.icon; return (<div key={trophy.id} className={`p-4 rounded-xl border flex flex-col items-center text-center gap-3 transition-all ${isUnlocked ? 'bg-[#111] border-gold/50 shadow-[0_0_15px_rgba(212,175,55,0.1)]' : 'bg-black border-white/5 opacity-50 grayscale'}`}><div className={`p-3 rounded-full ${isUnlocked ? 'bg-gold/10 text-gold' : 'bg-gray-900 text-gray-600'}`}>{isUnlocked ? <TrophyIcon className="w-6 h-6" /> : <Lock className="w-6 h-6" />}</div><div><h3 className={`text-sm font-bold ${isUnlocked ? 'text-white' : 'text-gray-500'}`}>{trophy.title}</h3><p className="text-[10px] text-gray-500 mt-1 leading-tight">{trophy.desc}</p></div></div>); })}</div></div>
        </div></PageTransition>
    );
}
...
function ProjectScreen({ onBack }) { 
    const [projects, setProjects] = useState(() => { const saved = localStorage.getItem('imperium_projects'); if (saved) return JSON.parse(saved); const oldName = localStorage.getItem('imperium_project_name'); const oldTasks = JSON.parse(localStorage.getItem('imperium_tasks') || "[]"); if (oldName) { return [{ id: Date.now(), title: oldName, deadline: "", tasks: oldTasks, answers: {} }]; } return []; });
    const [activeProject, setActiveProject] = useState(null); 
    const [newProjectName, setNewProjectName] = useState("");
    const [newProjectDeadline, setNewProjectDeadline] = useState("");
    const [newTask, setNewTask] = useState("");
    useEffect(() => { localStorage.setItem('imperium_projects', JSON.stringify(projects)); }, [projects]);
    const addProject = (e) => { e.preventDefault(); if (!newProjectName.trim()) return; setProjects([...projects, { id: Date.now(), title: newProjectName, deadline: newProjectDeadline, tasks: [], answers: {} }]); setNewProjectName(""); setNewProjectDeadline(""); };
    const deleteProject = (id, e) => { e.stopPropagation(); if(confirm("Confirmer l'abandon de ce front ?")) { setProjects(projects.filter(p => p.id !== id)); if(activeProject && activeProject.id === id) setActiveProject(null); } };
    const addTask = (e) => { e.preventDefault(); if (!newTask.trim() || !activeProject) return; const updatedProjects = projects.map(p => { if (p.id === activeProject.id) { return { ...p, tasks: [...(p.tasks || []), { id: Date.now(), text: newTask, done: false }] }; } return p; }); setProjects(updatedProjects); setActiveProject(updatedProjects.find(p => p.id === activeProject.id)); setNewTask(""); }; 
    const toggleTask = (taskId) => { const updatedProjects = projects.map(p => { if (p.id === activeProject.id) { const newTasks = p.tasks.map(t => t.id === taskId ? { ...t, done: !t.done } : t); return { ...p, tasks: newTasks }; } return p; }); setProjects(updatedProjects); setActiveProject(updatedProjects.find(p => p.id === activeProject.id)); }; 
    const deleteTask = (taskId) => { const updatedProjects = projects.map(p => { if (p.id === activeProject.id) { return { ...p, tasks: p.tasks.filter(t => t.id !== taskId) }; } return p; }); setProjects(updatedProjects); setActiveProject(updatedProjects.find(p => p.id === activeProject.id)); };
    const updateAnswer = (qId, value) => { const updatedProjects = projects.map(p => { if (p.id === activeProject.id) { return { ...p, answers: { ...(p.answers || {}), [qId]: value } }; } return p; }); setProjects(updatedProjects); setActiveProject(updatedProjects.find(p => p.id === activeProject.id)); };

    if (!activeProject) {
        return (
            <PageTransition><div className="h-[100dvh] w-full max-w-md mx-auto bg-dark text-gray-200 font-sans flex flex-col overflow-hidden">
                <div className="shrink-0 px-5 py-4 bg-[#151515] border-b border-white/5 pt-[env(safe-area-inset-top)] z-10"><button onClick={onBack} className="flex items-center gap-2 text-gray-500 hover:text-white mb-4 mt-2"><ArrowLeft className="w-4 h-4" /> <span className="text-xs uppercase tracking-widest">Retour au QG</span></button><h1 className="text-2xl font-serif text-white font-bold">Conquêtes</h1><p className="text-[10px] text-gray-500 mt-1">Gérez vos fronts actifs.</p></div>
                <div className="flex-1 overflow-y-auto p-5 pb-20 space-y-4 custom-scrollbar">{projects.length === 0 && <div className="text-center p-10 opacity-50"><Castle className="w-12 h-12 mx-auto mb-4 text-gray-600"/><p className="text-sm">Aucune conquête en cours.</p></div>}{projects.map(p => { const pTasks = p.tasks || []; const progress = pTasks.length === 0 ? 0 : Math.round((pTasks.filter(t => t.done).length / pTasks.length) * 100); const daysLeft = getDaysLeft(p.deadline); return (<div key={p.id} onClick={() => setActiveProject(p)} className="bg-[#111] border border-white/5 p-5 rounded-xl active:scale-[0.98] transition-all cursor-pointer group hover:border-gold/30"><div className="flex justify-between items-start mb-3"><div><h3 className="text-white font-bold font-serif text-lg">{p.title}</h3>{daysLeft !== null && (<p className={`text-[10px] font-bold ${daysLeft < 3 ? 'text-red-500 animate-pulse' : 'text-gray-500'}`}>{daysLeft > 0 ? `${daysLeft} Jours restants` : "Date dépassée"}</p>)}</div><button onClick={(e) => deleteProject(p.id, e)} className="text-gray-600 hover:text-red-500"><Trash2 className="w-4 h-4"/></button></div><div className="w-full bg-gray-900 rounded-full h-1.5 mb-3"><div className="h-full bg-gold rounded-full" style={{ width: `${progress}%` }}></div></div><div className="flex justify-between items-center text-[10px] text-gray-500"><span>{pTasks.length} Missions</span><span>{progress}%</span></div></div>); })}</div>
                <div className="shrink-0 p-4 bg-dark border-t border-white/10 pb-[calc(1rem+env(safe-area-inset-bottom))] max-w-md mx-auto w-full"><form onSubmit={addProject} className="flex gap-2"><input type="text" value={newProjectName} onChange={(e) => setNewProjectName(e.target.value)} placeholder="Nouveau Front..." className="flex-1 bg-[#111] border border-white/10 rounded-lg px-4 py-3 text-white text-sm focus:border-gold focus:outline-none" /><input type="date" value={newProjectDeadline} onChange={(e) => setNewProjectDeadline(e.target.value)} className="bg-[#111] border border-white/10 rounded-lg px-2 text-white text-xs focus:border-gold outline-none" /><button type="submit" disabled={!newProjectName.trim()} className="bg-gold text-black font-bold p-3 rounded-lg disabled:opacity-50 hover:bg-yellow-400 transition-colors"><Plus className="w-5 h-5" /></button></form></div>
            </div></PageTransition>
        );
    }
    ...
    return (
        <PageTransition><div className="h-[100dvh] w-full max-w-md mx-auto bg-dark text-gray-200 font-sans flex flex-col overflow-hidden">
            <div className="shrink-0 px-5 py-4 bg-[#151515] border-b border-white/5 pt-[env(safe-area-inset-top)] z-10"><button onClick={() => setActiveProject(null)} className="flex items-center gap-2 text-gray-500 hover:text-white mb-4 mt-2"><ArrowLeft className="w-4 h-4" /> <span className="text-xs uppercase tracking-widest">Retour aux Conquêtes</span></button><h1 className="text-2xl font-serif text-white font-bold">{activeProject.title}</h1><div className="flex items-center gap-4 mt-4"><div className="flex-1 h-2 bg-gray-800 rounded-full overflow-hidden"><div className="h-full bg-gold transition-all duration-500" style={{ width: `${progress}%` }}></div></div><span className="text-gold font-bold text-sm">{progress}%</span></div></div>
            <div className="flex-1 overflow-y-auto p-5 pb-32 custom-scrollbar"><div className="mb-8 space-y-4"><div className="flex items-center gap-2 mb-2 opacity-80"><Lightbulb className="w-4 h-4 text-gold" /><h3 className="text-xs font-bold uppercase tracking-widest text-gold">Interrogatoire Tactique</h3></div>{STRATEGIC_QUESTIONS.map(q => (<div key={q.id} className="bg-[#111] border border-white/5 p-4 rounded-lg"><p className="text-xs text-gray-400 mb-2 italic">{q.q}</p><input type="text" value={(activeProject.answers && activeProject.answers[q.id]) || ""} onChange={(e) => updateAnswer(q.id, e.target.value)} placeholder="Réponse stratégique..." className="w-full bg-black border-b border-white/10 text-white text-sm py-1 focus:border-gold focus:outline-none placeholder-gray-800" /></div>))}</div><div className="flex items-center gap-2 mb-4 opacity-80"><CheckSquare className="w-4 h-4 text-white" /><h3 className="text-xs font-bold uppercase tracking-widest text-white">Plan de Bataille</h3></div><div className="space-y-3">{pTasks.map(task => (<div key={task.id} className={`flex items-start gap-3 p-3 rounded-lg border transition-all ${task.done ? 'bg-dark border-transparent opacity-50' : 'bg-[#111] border-white/5'}`}><button onClick={() => toggleTask(task.id)} className="mt-0.5 text-gold hover:scale-110 transition-transform">{task.done ? <CheckSquare className="w-5 h-5" /> : <Square className="w-5 h-5" />}</button><p className={`flex-1 text-sm ${task.done ? 'line-through text-gray-600' : 'text-gray-200'}`}>{task.text}</p><button onClick={() => deleteTask(task.id)} className="text-gray-700 hover:text-red-500"><X className="w-4 h-4" /></button></div>))}{pTasks.length === 0 && <p className="text-gray-600 text-xs italic">Aucune mission définie. L'ennemi avance.</p>}</div></div>
            <div className="shrink-0 p-4 bg-dark border-t border-white/10 pb-[calc(1rem+env(safe-area-inset-bottom))] max-w-md mx-auto w-full"><form onSubmit={addTask} className="flex gap-2"><input type="text" value={newTask} onChange={(e) => setNewTask(e.target.value)} placeholder="Nouvelle mission..." className="flex-1 bg-[#111] border border-white/10 rounded-lg px-4 py-3 text-white text-sm focus:border-gold focus:outline-none" /><button type="submit" disabled={!newTask.trim()} className="bg-gold text-black font-bold p-3 rounded-lg disabled:opacity-50 hover:bg-yellow-400 transition-colors"><Plus className="w-5 h-5" /></button></form></div>
        </div></PageTransition>
    ); 
}
...
function SettingsScreen({ onBack }) { 
    ...
    const handleExport = () => { const data = { balance: localStorage.getItem('imperium_balance'), transactions: localStorage.getItem('imperium_transactions'), projects: localStorage.getItem('imperium_projects'), tasks: localStorage.getItem('imperium_tasks'), skills: localStorage.getItem('imperium_skills'), currency: localStorage.getItem('imperium_currency'), zone: localStorage.getItem('imperium_zone'), onboarded: localStorage.getItem('imperium_onboarded'), bunker: localStorage.getItem('imperium_bunker'), version: localStorage.getItem('imperium_version'), license: localStorage.getItem('imperium_license') }; const encoded = btoa(JSON.stringify(data)); setExportCode(encoded); navigator.clipboard.writeText(encoded).then(() => alert("CODE D'ARCHIVE COPIÉ.")).catch(() => alert("Copie automatique échouée. Veuillez copier le code affiché manuellement.")); }; 
    const handleImport = () => { try { if(!importData) return; const decoded = JSON.parse(atob(importData)); if(decoded.balance) localStorage.setItem('imperium_balance', decoded.balance); if(decoded.transactions) localStorage.setItem('imperium_transactions', decoded.transactions); if(decoded.projects) localStorage.setItem('imperium_projects', decoded.projects); if(decoded.project) localStorage.setItem('imperium_project_name', decoded.project); if(decoded.tasks) localStorage.setItem('imperium_tasks', decoded.tasks); if(decoded.skills) localStorage.setItem('imperium_skills', decoded.skills); if(decoded.currency) localStorage.setItem('imperium_currency', decoded.currency); if(decoded.zone) localStorage.setItem('imperium_zone', decoded.zone); if(decoded.onboarded) localStorage.setItem('imperium_onboarded', decoded.onboarded); if(decoded.bunker) localStorage.setItem('imperium_bunker', decoded.bunker); if(decoded.version) localStorage.setItem('imperium_version', decoded.version); if(decoded.license) localStorage.setItem('imperium_license', decoded.license); alert("✅ RESTAURATION RÉUSSIE."); window.location.reload(); } catch (e) { alert("❌ ERREUR : Code invalide."); } }; 
    const handleRecalibrate = () => { if(confirm("Confirmer le recalibrage manuel des soldes ?")) { localStorage.setItem('imperium_balance', JSON.stringify(parseFloat(calibBalance) || 0)); localStorage.setItem('imperium_bunker', JSON.stringify(parseFloat(calibBunker) || 0)); alert("SYSTÈME RECALIBRÉ."); window.location.reload(); } };
    const resetEmpire = () => { if(confirm("DANGER : Voulez-vous vraiment TOUT effacer ?")) { localStorage.clear(); window.location.reload(); } }; 
    return (
        <PageTransition><div className="h-[100dvh] w-full max-w-md mx-auto bg-dark text-gray-200 font-sans flex flex-col overflow-hidden">
            <div className="shrink-0 px-5 py-4 bg-[#151515] border-b border-white/5 pt-[env(safe-area-inset-top)] z-10"><button onClick={onBack} className="flex items-center gap-2 text-gray-500 hover:text-white mb-4 mt-2"><ArrowLeft className="w-4 h-4" /> <span className="text-xs uppercase tracking-widest">Retour au QG</span></button><h1 className="text-2xl font-serif text-white font-bold">Paramètres</h1></div>
            <div className="flex-1 overflow-y-auto p-5 space-y-8 custom-scrollbar"><div className="bg-[#1a1a1a] p-5 rounded-xl border border-white/5 relative overflow-hidden"><div className="absolute top-0 right-0 p-4 opacity-5"><RefreshCw className="w-24 h-24 text-white" /></div><div className="flex items-center gap-3 mb-4 relative z-10"><div className="p-2 bg-white/10 text-white rounded-lg"><RefreshCw className="w-5 h-5"/></div><div><h3 className="text-sm font-bold text-gray-200">Calibrage du Système</h3><p className="text-[10px] text-gray-500">Correction manuelle des soldes.</p></div></div><div className="space-y-4 relative z-10"><div><label className="text-[10px] uppercase text-gray-500 font-bold mb-1 block">Solde Cash/OM Actuel</label><input type="number" value={calibBalance} onChange={(e) => setCalibBalance(e.target.value)} className="w-full bg-black border border-white/10 rounded-lg p-3 text-white focus:border-gold outline-none" /></div><div><label className="text-[10px] uppercase text-gray-500 font-bold mb-1 block">Solde Wave (Bunker) Actuel</label><input type="number" value={calibBunker} onChange={(e) => setCalibBunker(e.target.value)} className="w-full bg-blue-900/10 border border-blue-500/20 rounded-lg p-3 text-white focus:border-blue-500 outline-none" /></div><button onClick={handleRecalibrate} className="w-full bg-white/10 hover:bg-white/20 text-white border border-white/10 font-bold py-3 rounded-lg text-xs uppercase tracking-widest transition-colors">Appliquer les Corrections</button></div></div><div className="bg-[#111] p-5 rounded-xl border border-white/5"><div className="flex items-center gap-3 mb-3"><div className="p-2 bg-blue-900/20 text-blue-400 rounded-lg"><Download className="w-5 h-5"/></div><div><h3 className="text-sm font-bold text-gray-200">Sauvegarder l'Empire</h3><p className="text-[10px] text-gray-500">Générez un code unique.</p></div></div><button onClick={handleExport} className="w-full bg-blue-600/20 hover:bg-blue-600/40 text-blue-400 border border-blue-500/30 font-bold py-3 rounded-lg text-xs uppercase tracking-widest flex items-center justify-center gap-2 transition-colors mb-3"><Copy className="w-4 h-4" /> Générer Code</button>{exportCode && (<div className="animate-in fade-in slide-in-from-top-2"><p className="text-[10px] text-gray-500 mb-1">Copiez ce code manuellement si besoin :</p><textarea readOnly value={exportCode} className="w-full h-24 bg-black border border-blue-500/30 rounded-lg p-2 text-[10px] text-blue-300 font-mono focus:outline-none" onClick={(e) => e.target.select()}></textarea></div>)}</div><div className="bg-[#111] border border-white/5 rounded-xl p-5"><div className="flex items-center gap-3 mb-3"><div className="p-2 bg-green-900/20 text-green-400 rounded-lg"><Upload className="w-5 h-5"/></div><div><h3 className="text-sm font-bold text-gray-200">Restaurer les données</h3><p className="text-[10px] text-gray-500">Collez le code ici.</p></div></div><textarea value={importData} onChange={(e) => setImportData(e.target.value)} placeholder="Collez votre code ici..." className="w-full bg-black border border-white/10 rounded-lg p-3 text-xs text-gray-300 focus:border-gold focus:outline-none h-20 mb-3 font-mono"/><button onClick={handleImport} disabled={!importData} className="w-full bg-green-600/20 hover:bg-green-600/40 text-green-400 border border-green-500/30 font-bold py-3 rounded-lg text-xs uppercase tracking-widest disabled:opacity-50 transition-colors">Restaurer</button></div><div className="pt-10 border-t border-white/5"><button onClick={resetEmpire} className="w-full flex items-center justify-center gap-2 text-red-500 hover:text-red-400 text-xs uppercase tracking-widest py-4 hover:bg-red-900/10 rounded-lg transition-colors"><Trash2 className="w-4 h-4" /> Détruire l'Empire (Reset)</button></div></div>
        </div></PageTransition>
    ); 
}